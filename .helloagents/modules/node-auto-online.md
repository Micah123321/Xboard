...
- `Server` 模型的 `last_check_at`、`last_push_at`、在线用户、metrics 和负载状态访问器按节点自身 ID 读取缓存；子节点不再通过 `parent_id` 回退读取父节点运行缓存。
...
- 子节点的自动上线、墙检测否决和重连冷却只按子节点自身状态判定；父节点离线或不可直连不会通过自动上线流程隐藏转发入口子节点。
...
- 依赖 `app/Models/Server.php` 提供按节点自身读取的运行状态访问器
