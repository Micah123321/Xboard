...
- 自动上线写入 `show` 时使用当前节点的 `available_status` 判断有效运行状态；普通节点只读取自身运行缓存，转发子节点在自身缓存缺失或过期时默认可回退父节点运行缓存；子节点开启 `tcp_check_enabled` 后不再回退父节点缓存，只认自身 TCP 检测写入的运行状态。
...
- 子节点 `available_status`、在线用户、metrics 和负载状态默认可使用父入口运行缓存兜底，避免管理端展示和自动上线把仅由父入口上报的转发入口判定为离线；开启 `tcp_check_enabled` 的子节点会关闭该兜底，`sync:server-tcp-checks` 每分钟检测 `host:server_port`，连通写自身 `LAST_CHECK_AT`，失败清理自身 `LAST_CHECK_AT` / `LAST_PUSH_AT` 并触发当前子节点自动上线重算；墙检测否决、重连冷却和 `show` 写入仍只作用于当前子节点，不恢复父节点对子节点的批量显隐联动。
...
- 依赖 `app/Services/ServerTcpCheckService.php` 为开启 TCP 检测的转发子节点写入或清理自身运行缓存
...
- 依赖 `app/Console/Commands/SyncServerAutoOnline.php`、`app/Console/Commands/SyncServerTcpChecks.php` 与 Laravel Scheduler 执行兜底同步
