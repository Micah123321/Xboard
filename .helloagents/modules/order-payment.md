...
- 后台 `OrderController::batchConfirmCommission()` 负责批量确认邀请佣金；接口在事务内锁定本次提交的订单，只允许 `status=3`、`commission_status=0`、`invite_user_id IS NOT NULL` 且 `commission_balance>0` 的订单更新为发放中，其他订单计入跳过结果
...
- 依赖 `admin-frontend/src/views/subscriptions/OrdersView.vue`、`OrderDetailDrawer.vue` 与 `src/utils/orders.ts` 展示后台支付成功信息和佣金确认状态
