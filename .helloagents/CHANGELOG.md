...
### 新增
- **[admin-frontend]**: 节点管理页业务列新增三态字段排序，支持节点 ID、显隐、墙检测、自动上线、节点、地址、在线人数、倍率和权限组按“置顶 / 置底 / 默认”循环切换；默认态继续保留后端 `sort` 顺序，并抽出 `NodeSortableHeader` 复用表头交互 — by yinjianm
  - 方案: [202605181800_node-field-sort](archive/2026-05/202605181800_node-field-sort/)
  - 决策: node-field-sort#D001(使用前端本地三态排序)
