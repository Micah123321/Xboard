...
- 节点自动上线由 `ServerAutoOnlineService` 统一编排；节点可开启 `auto_online_cooldown_enabled` 重连冷却，在 1 小时内在线 / 离线切换超过 10 次后隐藏 6 小时，冷却期内自动上线不会重新显示该节点；子节点运行状态缓存按子节点自身读取，不再继承父节点 `last_check_at` / `last_push_at`
- 管理端套餐管理现已接入:
  - `plan/fetch`
  - `plan/save`
  - `plan/update`
